...
public class TaskJpaResource {
    ...
    public ResponseEntity<Object> updateTask(@PathVariable int id, @RequestBody Task updatedTask) {
    ...
    @PutMapping("/jpa/task/status/{id}")
    public ResponseEntity<Object> updateTaskStatus(@PathVariable int id, @RequestBody boolean isCompleted) {
        Optional<Task> optionalTask = taskRepository.findById(id);

        if(optionalTask.isEmpty())
            throw new TaskNotFoundException("id:"+id);
        Task savedTask = optionalTask.get();
        savedTask.setCompleted(isCompleted);
    ...
    @PutMapping("/jpa/task/duedate/{id}")
    public ResponseEntity<Object> updateTaskDueDate(@PathVariable int id, @RequestBody LocalDateTime newDate) {
        Optional<Task> optionalTask = taskRepository.findById(id);

        if(optionalTask.isEmpty())
            throw new TaskNotFoundException("id:"+id);
        Task savedTask = optionalTask.get();
        savedTask.setDueDate(newDate);
